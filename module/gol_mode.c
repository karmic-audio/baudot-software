...
void process_gol_keys(uint8_t k, uint8_t m, bool is_held_key)
{
    ...
    else if (match_no_mod(m, k, HID_ENTER)) 
    {
        ...
        if (gol_cell_isalive(ss_gol_grid_ptr(&scene_state), GolVarX, GolVarY) == 1){
            gol_cell_off(ss_gol_grid_ptr(&scene_state), GolVarX, GolVarY);
        }
        else {gol_cell_on(ss_gol_grid_ptr(&scene_state), GolVarX, GolVarY);}
        dirty=true;
    }
    ...
    else if (no_mod(m) && k >= HID_1 && k <= HID_0)
    {
        ...
        gol_cell_tr(&scene_state, GolVarX, GolVarY, n);
        dirty = true;
        }
    }
}
    ...
    {   
        ...
        //3.Render Cells
            for (uint8_t i = 0; i < GOL_X; i++)
            {
                ...
                {
                    if (gol_get_tr(&scene_state, i, j))
                    {
                        uint8_t y = j - (j / 4) * 4;
                        region_fill_part(&line[j / 4], (i * 2) + (y * 2 * 128), 2, 2);
                        region_fill_part(&line[j / 4], (i * 2) + (y * 2 * 128) + 128, 2, 2);
                    }
                    if (gol_cell_isalive(ss_gol_grid_ptr(&scene_state), i, j) == 1)
                    {
                        uint8_t y = j - (j / 4) * 4;
                        region_fill_part(&line[j / 4], (i * 2) + (y * 2 * 128), 2, 6);
                        region_fill_part(&line[j / 4], (i * 2) + (y * 2 * 128) + 128, 2, 6);
                    }
                    
                }
            }
            region_fill_part(&line[CursorTLine], CursorVarX + CursorVarY * 128, 2, 12);
